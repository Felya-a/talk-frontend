...
export default function useWebRTC(room: Room) {
	...
	useEffect(() => {
		...
		return () => {
			socketService.off(InputMessagesTypes.ADD_PEER)
		}
	}, [])
	...
	useEffect(() => {
		...
		return () => {
			socketService.off(InputMessagesTypes.SESSION_DESCRIPTION)
		}
	}, [])

	useEffect(() => {
		async function handleIceCandidate({ peer_id: peerID, ice_candidate: iceCandidate }) {
		...
		socketService.on(InputMessagesTypes.ICE_CANDIDATE, handleIceCandidate)

		return () => {
			socketService.off(InputMessagesTypes.ICE_CANDIDATE)
		}
	}, [])
	...
	useEffect(() => {
		...
		return () => {
			socketService.off(InputMessagesTypes.REMOVE_PEER)
		}
	}, [])
	...
	useEffect(() => {
		async function startCapture() {
			...
			// INFO: Нужно вызывать именно тут чтобы успели зарегистрироваться динамические обработчики из этого хука
		...
		return () => {
			...
			// TODO: Подумать нужно ли оно тут или вызывать в другом месте
